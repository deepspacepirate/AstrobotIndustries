...
var TxtType = function(el, toRotate, period, speed, loop, mode) {
	...
	this.period = parseInt(period, 10) || 1300; // pause after line
	...
	switch(this.mode){
		...
		case (modes.Append):
			...
			if (this.txtPos == 0) {
				...
				this.el = document.getElementsByClassName('wrap')[this.loopNum];
			}
...
window.startTerminal = function(){
	...
	for (var i = 0; i < elements.length; i++) {
		...
		if (toRotate) {
		  activeTxtType = new TxtType(elements[i], JSON.parse(toRotate), period, speed, loop, mode);
		}
	}
}
